return {
    ["REBEL"] = {
        ["SAIJO"] = {"Nantz_Independence", "Mon_Mothma_Team"},
        ["ENDOR"] = {"Han_Solo_Team", "Luke_Skywalker_Jedi_Team", "Home_One", "Lando_Calrissian_Team"},
        ["MON_CALAMARI"] = {"Airen_Cracken_Team", "Katarn_Team"},
        ["SULLUST"] = {"Sovv_Dauntless"},
    },
    ["EMPIREOFTHEHAND"] = {
        ["NIRAUAN"] = {
            "Thrawn_Grey_Wolf",
            "Niriz_Admonitor",
            "Parck_Strikefast",
            "Hand_of_Judgement_Team"
        },
        ["SYCA"] = {
			"Aralani_Frontier",
			"Stent_Thrawns_Hand",
			"Tasse_Team"}
    },
    ["GREATER_MALDROOD"] = {
        ["CENTARES"]  = {"Neomen_Ion_Storm", "Treuten_13X", "Therbon_Allegiance", "Somos_Team", "Tanniel_Team"},
        ["HAKASSI"] = {"Kosh_Lancet"},
		["ROCHE"] = {"Nervi_Blood_Ambition"},
		["THANOS"] = {"Trier_Secutor", "Maston_Team", "Ramier_Team", "Lacmar_Team"}
    },
    ["PENTASTAR"] = {
        ["ENTRALLA"]  = {"Grant_Oriflamme", "Trico_Pula", "Major_Noda_Team", "Brazack_Team"},
        ["BASTION"]  = {"Sariss_Team", "Jerec_Team", "Slosin_Ironhand"},
    },
    ["ERIADU_AUTHORITY"] = {
        ["ERIADU"]  = {"Veers_AT_AT_Team", "Delvardus_Brilliant", "Nyrox_Team", "Malcom_Team", "Grath_Stormtrooper_Team"},
		["SANRAFSIX"]  = {"Resuun_Retribution", "Hublin_Kabalian_Cross", "Johans_Team", "Adrick_Team"},
		["YAGDHUL"]  = {"Prittick_Tyrant", "Octavion_Sorin_Team", "Snopel_Team"}
    },
    ["ZSINJ_EMPIRE"] = {
        ["DATHOMIR"]  = {
            "Zsinj_Iron_Fist_VSD",
            "Joshi_Provocateur",				
            "Netbers_Team",
			"Melvar_Team",
            "Gethzerion_Team"
        },
		["PHINDAR"] = {"Screed_Demolisher"},
		["SERENNO"] = {
			"Banjeer", 
			"Lanu_Team", 
			"Teubbo_Team"
		},
		["STENOS"] = {
			"Trigit_Implacable",
			"Nabyl_Hawkbat",
			"Night_Caller"
		}
    },
	["CORPORATE_SECTOR"] = {
        ["ETTI"]  = {
			"Grumby_Notropis",
			"Karrek_Flim_Team",
			"Riga_Team",
			"Fasser_Team"
		},
		["OSLUMPEX"] = {
			"Sloane_Enforce",
			"Brollyx_Dauntless",
			"Odumin_Team",
			"Nieler_Team"
		},
		["MARYO"] = {
			"Weplinn_ISD",
			"Krin_Invincible",
			"Fiolla_Team"
		}
	},
	["HAPES_CONSORTIUM"] = {
        ["HAPES"]  = {
			"Isolder_Song_of_War",
			"Taa_Chume_Star_Home",
            "Gray_Kendall",
            "Chelik_Dangerous_Dance",
            "Alyssia_Starsong"
		},
        ["CHARUBAH"]  = {
            "Beed_Thane_Team",
            "Lovett_Riverlord",
            "Livette_Team"
        }
	},
	["HUTT_CARTELS"] = {
		["GAMORR"]  = {
			"Zorba_The_Hutt_Team"
		},
        ["LATHARRA"]  = {
			"Borvo_Prosperous_Secret"
		},
		["CIRCUMTORE"]  = {
			"Tronox_Team",
			"Reshul_Team",
        },
		["MULATAN"]  = {
            "Troonol_Agrelcu_Haalta"
        },
        ["NAL_HUTTA"]  = {
			"Bossato_Team",
			"Plakot_Team",
        },
		["NAR_SHADDAA"]  = {
            "Popara_Imru_Ootmian",
			"Parella_Team",
			"Smebba_Dunk_Team",
        },
		["UBRIKKIA"]  = {
            "Quaffug_Lorda"
        }
	},
    ["YEVETHA"] = {
        ["NZOTH"] = {
            "Spaar_Aramadia_SSD",
            "Fraan_Glory",
            "Ton_Raalk_Team",
            "Vor_Duull_Team"
        }
    },
}

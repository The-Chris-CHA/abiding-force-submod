...
--*   @Filename:            GCCorporateAcquisitions.lua
--*   @Last modified by:
...
function Find_Faction(message)
    if message == OnEnter then
		...
		elseif p_hutts.Is_Human() then
            Story_Event("ENABLE_BRANCH_HUTTS_FLAG")
        end
	else
		crossplot:update()
    end
...
function Initialize(message)
    if message == OnEnter then
		...
		if techLevel == 2 or techLevel == 3 then
			crossplot:publish("empty", "empty") --I don't like this solution any more than you do, but we don't have time for anything better. ~Mord
			crossplot:publish("empty", "empty")
			crossplot:publish("empty", "empty")
			crossplot:publish("empty", "empty")
			crossplot:publish("empty", "empty")
			crossplot:publish("empty", "empty")
			...
			Set_Fighter_Hero("WRAITH_SQUADRON_SPACE", "HAN_SOLO_MON_REMONDA")
			crossplot:publish("NR_FILTER_ADD", {"Wedge_Wraith_Location_Set"}, 2)
		end
		crossplot:update()
	else
		crossplot:update()
...
function Era_Setup(message)
    if message == OnEnter then
		...
		elseif techLevel == 7 then
			Story_Event("START_TEMPEST_FEUD")
        else
            --CSA_extra_forces()
            --orron_forces()
            --mytus_forces()
